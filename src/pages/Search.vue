<template>
    <div id="search">
        ...
        <div class="searchBar">
            ...
            <el-popover ref="popover4"
                ...
                <ul class="searchList"
                    ...
                    <li class="searchListItems"
                        ...
                        <i class="el-icon-delete fr"
                           @click.prevent="clearThisHistory(index)"></i>
                    </li>
                    <div class="clearHistory"
                         @click="clearHistory">清空历史记录</div>
                </ul>
                <ul class="noHistory"
                    v-show="test.length == 0">
                    暂无历史搜索记录
            ...
            <span class="toutiaoIcon"
                  v-popover:popover4>历史记录</span>
        </div>
        <div v-show="loading"
             v-loading="loading"
        ...
        <div class="searchNews">
            <nuxt-link v-for="(val,index) in con"
                       :to="{
                    name:'newsdetail',
                    params:
                        {   id:val.tag_id,
                ...
                <p class="title"
                   v-html="replace(val.title,input)"></p>
                <div>
                    <img alt="加载出错"
                         v-for="(img,index) in val.image_list"
                         :key="index"
                         v-lazy="img.url">
                    <div class="bottomInfo clearfix">
                        <Icon type="fireball"
                              size="10"
                              color="#d43d3d"
                              v-show="val.hot===1"></Icon>
                        <span class="avIcon"
                              v-show="val.label==='广告'">广告</span>
